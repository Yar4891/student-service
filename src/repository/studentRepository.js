...
export const findStudentById = id => Student.findById(id).select({password: 0}).exec(); // or '-password'
...
export const findStudentsByMinScore = (exam, minScore) =>
    Student.find({[`scores.${exam}`]: {$gte: minScore}}).select({password: 0}).exec();
