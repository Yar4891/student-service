...
export const findStudentById = id => Student.findById(id).select({password: 0}).lean().exec(); // or '-password'
...
export const updateStudent = (id, data) => Student.findByIdAndUpdate(id, data,
    {returnDocument: 'after'}).select({scores: 0}).lean().exec();

export const findStudentsByName = name => Student.find({name: new RegExp(`^${name}$`,
        'i')}).select({password: 0}).lean().exec();
...
export const findStudentsByMinScore = (exam, minScore) =>
    Student.find({[`scores.${exam}`]: {$gte: minScore}}).select({password: 0}).lean().exec();
